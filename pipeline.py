...
class cas12_iterative_refinement_pipeline:
    ...
    def extract_target_pdbs(self, template_csv, outdir):
        ...
        target_list = os.path.join(outdir, "target.lst")
        templates.target_id.to_csv(target_list, header=None, index=False)
        cmd = f"{self.params['foldcomp_program']} decompress -t 10 -y --id-list {target_list} {self.params['foldcomp_database']} {outdir} 1>/dev/null 2>&1"
        os.system(cmd)
...
class cas12_iterative_refinement_pipeline_controller:
...
def run_cas12_refinement_pipeline(params, refinement_inputs, outdir, finaldir, prefix):
    pipeline = cas12_iterative_refinement_pipeline_controller(params=params)
