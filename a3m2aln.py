...
CALNF_PATH = "/share/share_data/DeepMSA_tools/DeepMSA2/bin/calNf"
...
def merge_msa(msas: Sequence[Msa]):
    seen_sequences = set()
    ...
    return list(seen_sequences)


def single_sequence_a3m2aln(a3m_seq):
    aln_seq = ''
    remove_lower = lambda text: re.sub('[a-z]', '', text)
    aln_seq = remove_lower(a3m_seq)
    return aln_seq


def main(args, starmark_remove=True):
    merged_msa_list = []
    with open(args.bfd_a3m, 'r') as f:
        bfd_a3m = parse_a3m(f.read())
        merged_msa_list.append(bfd_a3m)
    if args.uniref_sto:
        with open(args.uniref_sto, 'r') as f:
            uniref_sto = parse_stockholm(f.read())
            merged_msa_list.append(uniref_sto)
    if args.mgnify_sto:
        with open(args.mgnify_sto, 'r') as f:
            mgnify_sto = parse_stockholm(f.read())
            merged_msa_list.append(mgnify_sto)
    msa_seq = merge_msa(merged_msa_list)
    if args.aln_out:
        aln_out = open(args.aln_out, 'w')
        for seq in msa_seq:
            aln_seq = single_sequence_a3m2aln(seq.strip("\n"))
            if starmark_remove:
                aln_out.write('%s\n' % aln_seq.replace('*', ''))
            else:
                aln_out.write('%s\n' % aln_seq)
        ...
        for seq in msa_seq:
            aln_seq = single_sequence_a3m2aln(seq.strip("\n"))
            if starmark_remove:
                print('%s' % aln_seq.replace('*', ''))
            else:
                print('%s' % aln_seq)
    if args.cal_neff:
        cmd = f"{CALNF_PATH} {args.aln_out}"
        os.system(cmd)


if __name__ == '__main__':
    argparser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    argparser.add_argument("--bfd_a3m", type=str, required=True, help="The bfd a3m file")
    argparser.add_argument("--uniref_sto", type=str, required=False, help="The uniref sto file")
    argparser.add_argument("--mgnify_sto", type=str, required=False, help="The mgnify sto file")
    argparser.add_argument("--aln_out", type=str, default=None, required=False, help="The aln out file")
    argparser.add_argument("--cal_neff", action='store_true', help="Whether to calculate neff value")
