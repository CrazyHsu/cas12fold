...
from alphafold.data import templates as af2_templates
...
flags.DEFINE_string('feature_pkl', None, 'Existing feature pkl')
...
def _reorder_chains(pdbstring):
    ...
    for line in pdbstring.split('\n'):
        if line.startswith('ATOM') or line.startswith('TER'):
            ...
            if first_chain_id != "A":
                new_pdbstring += [
                    line[:21] + protein.PDB_CHAIN_IDS[protein.PDB_CHAIN_IDS.find(chain_id) - 1] + line[22:]]
            else:
                new_pdbstring += [line]
        else:
            new_pdbstring += [line]
...
def predict_structure(
    ...
    for model_index, (model_name, model_runner) in enumerate(model_runners.items()):
        ...
        if not os.path.exists(unrelaxed_pdb_path) or not os.path.exists(result_output_path) or result_overwrite:
            ...
            if benchmark:
                ...
                logging.info('Total JAX model %s on %s predict time (excludes compilation time): %.1fs', model_name,
                             fasta_name, t_diff)
    ...
    for model_name in to_relax:
        ...
        if (not result_overwrite) and os.path.exists(relaxed_output_path):
...
def main(argv):
    ...
    custom_inputs = CustomizedInputs_Monomer()
    ...
    if not custom_inputs.notemplate:
        ...
        template_featurizer = af2_templates.HhsearchHitFeaturizer(
    ...
    monomer_data_pipeline = cas12fold_refiner_pipeline.DataPipeline(
        ...
        bfd_database_path=os.path.join(FLAGS.database_dir,
