#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @File   : cas12fold_refiner_templates.py
...
class CustomizedMonomerHitFeaturizer:
    ...
    def get_templates(self,
                      ...
                      hits_file: str,
                      template_select_foldseek_global: bool = False) -> TemplateSearchResult:
        ...
        template_hits_pd = pd.read_csv(hits_file, sep='\t')

        if template_select_foldseek_global:
            foldseek_local_templates = template_hits_pd[template_hits_pd['target'].str.contains('local')]
            foldseek_global_templates = template_hits_pd[~template_hits_pd['target'].str.contains('global')]
            tmp_foldseek_local_templates = foldseek_local_templates.head(int(self._max_hits * 0.75))
            tmp_foldseek_global_templates = foldseek_global_templates.head(int(self._max_hits * 0.25))
            pdb_hits_pd = pd.concat([tmp_foldseek_local_templates, tmp_foldseek_global_templates])
        else:
            pdb_hits_pd = template_hits_pd
